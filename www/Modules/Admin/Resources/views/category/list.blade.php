...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <form class="search-form" action="{{ route('admin.account.list') }}" method="GET"
                ...
                <table class="table table-striped table-bordered table-hover table-condensed">
                    ...
                    @foreach ($categories as $index => $category)
                        <tr>
                            ...
                            <td>{{ $category->name }}</td>
                            <td>
                                <a href="{{ route('admin.product.list', ['category_id' => $category->id]) }}"
                                   class="btn btn-info wid-80" title="Product list of category {{ $category->name }}">
                                    <i class="feather icon-list"></i>
                                </a>
                            </td>
                            <td>
                                <a href="{{ route('admin.product.add', ['category_id' => $category->id]) }}"
                                   class="btn btn-info wid-80" title="Add new product for {{ $category->name }}">
                                    <i class="feather icon-file-plus"></i>
                                </a>
                            </td>
                            <td>
                                <a href="{{ route('admin.category.update', ['category_id' => $category->id]) }}"
                                   class="btn btn-primary" title="Update">
                                    <i class="feather icon-edit"></i>
                                </a>
                                <button
                                    onclick="deleteRecord('{{ route('admin.category.delete', ['category_id' => $category->id]) }}')"
                                    data-href="{{ route('admin.category.delete', ['category_id' => $category->id]) }}"
