            ...
            <form class="validate-form" action="" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="row-input-group col-6 form-group">
                        ...
                        <input type="text" class="form-control" name="name" value="{{ $shop->name ?? old('name') }}"
                    ...
                    <div class="row-input-group col-6 form-group">
                        ...
                        <select class="form-control select-custom" name="account_id">
                            ...
                            @foreach ($accounts as $account)
                                <option @selected($account->id == $shop->account_id || $account->id == old('account_id')) value="{{ $account->id }}">{{ $account->full_name }}
                                </option>
                            @endforeach
                        </select>
                        @error('account_id')
